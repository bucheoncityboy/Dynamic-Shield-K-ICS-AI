"""
Phase 5.1: 분산 효과 최적화 Proof (이론적 타당성 검증)
===================================================
...
def prove_risk_paradox():
    ...
    """
    print("=" * 60)
    print("Phase 5.1: 분산 효과 최적화 Proof")
    print("=" * 60)
    ...
    for corr in correlations_to_test:
        ...
        print(f"  SCR Ratio at Optimal: {opt_scr:.4f}")
        print(f"  SCR Ratio at 100% Hedge: {scr_100:.4f}")
        print(f"  Capital Savings: {savings:.2f}%")
    ...
    print("\n" + "=" * 60)
    if paradox_count >= 3:
        print("[SUCCESS] 분산 효과 최적화 Proven!")
        print(f"  {paradox_count}/{len(results)} scenarios show the paradox")
    else:
        print("[PARTIAL] 분산 효과 최적화 partially demonstrated")
    ...
    plt.title('분산 효과 최적화: Why Less Hedge = Less Risk?')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('diversification_proof.png', dpi=150)
    plt.show()
    
    print("\n[Saved] diversification_proof.png")
